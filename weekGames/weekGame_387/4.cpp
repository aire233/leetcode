...
public:
    ...
    int greaterCount(int value) {
        return nums.end() - std::upper_bound(nums.begin(), nums.end(), value);
    }
...
public:
    vector<int> resultArray(vector<int>& nums) {
        ...
        vector<int> res1({nums[0]}), res2({nums[1]});
        for (int i = 2; i < nums.size(); i++) {
            int c1 = arr1.greaterCount(nums[i]);
            int c2 = arr2.greaterCount(nums[i]);
            if (c1 > c2 || (c1 == c2 && res1.size() <= res2.size())) {
                arr1.add(nums[i]);
